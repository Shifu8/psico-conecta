...
def create_app(config_class="app.config.Config", test_config=None):
    ...
    app.register_blueprint(bp, url_prefix="/api/pagos")
    app.register_blueprint(bp_webhooks, url_prefix="/api/pagos/webhooks")

    @app.get("/health")
    def health():
        try:
            db.session.execute(text("SELECT 1"))
            stripe_configurado = bool(app.config.get("STRIPE_SECRET_KEY"))
            modo = "test" if str(app.config.get("STRIPE_SECRET_KEY", "")).startswith("sk_test_") else "live"
            return jsonify(
                estado="ok",
                servicio="servicio-pagos",
                base_datos="ok",
                stripe="configurado" if stripe_configurado else "no_configurado",
                modo=modo if stripe_configurado else "sin_configurar",
            )
        except Exception:
            db.session.rollback()
            return jsonify(
    ...
    def error_integridad(_error):
        ...
        return jsonify(
            ...
            mensaje="El pago ya existe o entra en conflicto con otro registro.",
        ), 409
    ...
    def error_inesperado(error):
        db.session.rollback()
        app.logger.exception("Error en servicio-pagos: %s", error)
